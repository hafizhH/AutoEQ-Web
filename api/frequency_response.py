...
import re
import warnings
from .biquad import peaking,digital_coeffs,low_shelf
...
class FrequencyResponse:
    ...
    def optimize_biquad_filters(cls, frequency, target, max_time=5, max_filters=None, fs=DEFAULT_FS, fc=None, q=None):
        ...
        # Re-compute eq
        a0, a1, a2, b0, b1, b2 = peaking(_fc, _Q, _gain, fs=fs)
        frequency = np.repeat(np.expand_dims(frequency, axis=0), len(_fc), axis=0)
        _eq = np.sum(digital_coeffs(frequency, fs, a0, a1, a2, b0, b1, b2), axis=0)
    ...
    def write_eqapo_parametric_eq(self, file_path, filters, preamp=None):
        ...
        if preamp is None:
            ...
            for filt in filters:
                a0, a1, a2, b0, b1, b2 = peaking(filt[0], filt[1], filt[2], fs=44100)
                fr += digital_coeffs(self.frequency, 44100, a0, a1, a2, b0, b1, b2)
    ...
    def create_target(self,
        ...
        """
        bass_boost = digital_coeffs(
            self.frequency,
            DEFAULT_FS,
            *low_shelf(bass_boost_fc, bass_boost_q, bass_boost_gain, DEFAULT_FS)
        )
        if tilt is not None:
